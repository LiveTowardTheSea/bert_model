...
def get_model(config, tag_num,load_model,use_gpu):
    if config.model_name == 'bert':
        model = bert_model(config, tag_num)
    ...
    if load_model is not None:
        ...
        for name_, p in model.named_parameters():
            if p.dim() > 1 and str("encoder") not in name_:
...
def eval_model(model, my_data, mode='dev', use_gpu=True):
    ...
    with torch.no_grad():
        while True:
            try:
                sentence_tensor,sentence_mask,tag_tensor, tag_mask = data_iter.next()
                # 转换到gpu上面
                if use_gpu:
                    sentence_tensor = sentence_tensor.cuda()
                    tag_tensor = tag_tensor.cuda()
                    sentence_mask = sentence_mask.cuda()
                    tag_mask = tag_mask.cuda()
                ...
                decode_result = model.decoder(encoder_output, tag_mask, use_gpu)
...
def train_model(model, config, args, my_data, use_gpu):
    ...
    rest_params = filter(lambda x:id(x) not in decoder_param_id,model.parameters())
    optimizer = torch.optim.Adam([{'params':model.decoder.parameters(),'lr':config.lr},
                                {'params':rest_params,'lr':config.bert_lr}])
                                #weight_decay=config.regularization)
    # 实现的一些功能：lr decay / 模型保存
    # 求总共的步数
    total_steps = math.ceil(len(my_data.train_iter.data_list) / config.batch_size) * config.epoch_num
    ...
    for epoch_idx in range(config.epoch_num):
        ...
        while True:
            try:
                ...
                sentence_tensor,sentence_mask,tag_tensor, tag_mask = my_data.train_iter.next()
                ...
                if use_gpu:
                    ...
                    tag_mask = tag_mask.cuda()
                    sentence_mask = sentence_mask.cuda()
                batch_loss_tensor, batch_path = model(sentence_tensor, tag_tensor, sentence_mask, tag_mask, use_gpu)
                ...
                if config.clip_value is not None:
                    nn.utils.clip_grad_value_(model.parameters(),config.clip_value)
                optimizer.step()
                scheduler.step()
                batch_id += 1
                # eval_model(model, my_data, mode='dev', use_gpu=True)
...
if __name__ == '__main__':
    ...
    opt.add_argument('--model',choices=['bert'],default='bert')
    opt.add_argument('--status', choices=['train', 'test', 'decode'], default='train')
    opt.add_argument('--save_model', default='data/MSRA/saved_model/bert_crf')
    ...
    opt.add_argument('--load_data', help='load vocab or vector', default='data/MSRA')
    opt.add_argument('--train', help='the path of train file', default='data/MSRA/msra_train_bioes2')
    opt.add_argument('--dev', help='the path of dev file', default='data/MSRA/msra_dev_bioes2')
    opt.add_argument('--test', help='the path of test file', default='data/MSRA/msra_test_bioes2')
    args = opt.parse_args()
    # gpu 是否可以使用
    ...
    tag_num = data.tag_vocab.get_length()
    # 得到模型，这块，要根据究竟选择了哪些模型进行得到模型
    model = get_model(config, tag_num, args.load_model,use_gpu)
    # 下面 我们进行判断，进行模型训练
    if status == 'train':
